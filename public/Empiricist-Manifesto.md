...
2. **Slider Behavior**
   - User-Controlled Sliders:
     ...
     - Inflation Rate (i): [-1, 1], default 0
     - Basic Variables:
       - Constant Capital (c): [0, 10,000,000], default 100
       - Variable Capital (v): [0, 100,000], default 50
       - Surplus Value (s): [0, 100,000], default 50
     - Marxian Ratios:
       - Rate of Exploitation (s/v): [0, 50], default 1
       - Organic Composition (c/v): [0, 1,000], default 2
...
1. **State Updates**
   ...
   if (wageGrowth < 0) {
     ...
     const newC = totalValue - newV - newS;
...
1. **Discrete Value Change**: ΔV(t) represents concrete value changes over definite time periods
   - Δc(t) = a(t)·s(t) - d(t)·c(t) (constant capital increases through periodic accumulation, decreases through depreciation cycles)
...
1. **Constant Capital (c) Update**
   - Grows with inflation: c(t+1) = c(t) × (1 + inflation)
...
4. **Total Value (W) Update**
   - Sum of components: W(t+1) = c(t+1) + v(t+1) + s(t+1)
   - Maintains PT dimension
